...
class Module(object):
    ...
    def versioncourante(self, module_name):
        ...
        module_ids = []
        if module_name != 'all':
            module_id = connection.search('ir.module.module', [('name', '=',
                                                                module_name)])
            if module_id:
                module_ids.append(module_id[0])
        else:
            module_ids = connection.search('ir.module.module', [], 0, 100000,
                                           'name asc')
    ...
    def remove(self, module_name=None, module_id=None):
        ...
        model_data_ids = objet.execute(dbname, uid, pwd, 'ir.model.data',
        ...
        for model_data_id in model_data_ids:
            ...
            model_data = objet.execute(dbname, uid, pwd, 'ir.model.data',
                                       'read', model_data_id)
            try:
                objet.execute(dbname, uid, pwd, model_data['model'], 'unlink',
                              model_data['res_id'])
            except Exception as e:
                print("Erreur Supression ", model_data['model'], e)

            objet.execute(dbname, uid, pwd, 'ir.model.data', 'unlink', [id])
        objet.execute(dbname, uid, pwd, 'ir.module.module', 'write', module_id,
                      {'state': 'uninstalled'})
        objet.execute(dbname, uid, pwd, 'ir.module.module', 'unlink',
                      module_id)
    ...
    def uninstall(self, module_name=None, module_id=None):
        ...
        if module_id:
            ...
            module = objet.execute(dbname, uid, pwd, 'ir.module.module',
                                   'read', module_id)
            module_dependency_ids = objet.execute(
                dbname, uid, pwd, 'ir.module.module.dependency', 'search',
                [('name', '=', module['name'])])

            if module_dependency_ids:
                for module_dependency_id in module_dependency_ids:
                    module_dependency = objet.execute(
                        dbname, uid, pwd, 'ir.module.module.dependency',
                        'read', module_dependency_id)
                    dependant = objet.execute(
                        dbname, uid, pwd, 'ir.module.module', 'read',
    ...
    def update_list(self):
        ...
        pwd = self.connection.pwd
        try:
            res = objet.execute(dbname, uid, pwd, 'base.module.update',
                                'create', {})
            objet.execute(dbname, uid, pwd, 'base.module.update',
                          'update_module', res)
...
class Openerp(object):
    ...
    def clean_ir_model_data(self):
        ...
        model_ids = self.objet.execute(self.dbname, self.uid, self.pwd,
                                       'ir.model', 'search', [('model', '!=',
                                                               'res_users')])
        models = self.objet.execute(self.dbname, self.uid, self.pwd,
                                    'ir.model', 'read', model_ids, ['model'])
        modeles = sorted(models, key=lambda x: x['model'])
        for model in modeles:
            print(model['model'])
            model_data_ids = self.objet.execute(
                self.dbname, self.uid, self.pwd, 'ir.model.data', 'search',
                [('model', "=", model['model'])])
            for id in model_data_ids:
                model_data = self.objet.execute(
                    self.dbname, self.uid, self.pwd, 'ir.model.data', 'read',
                    id, ['model', 'res_id'])
                # ~ print model['model']
    ...
    def readall(self, module, fields=None, context=None):
        ids = self.objet.execute(self.dbname, self.uid, self.pwd, module,
                                 'search', [], 0, 10000000, 'name')
        return self.objet.execute(self.dbname, self.uid, self.pwd, module,
                                  'read', ids, fields, context)
